...
from model.original_trainer import NeuralUtilityTrainer
import numpy as np
...
MODEL_NAME = "mlp_choice_{}_{}".format(args.dataset, args.loss)
...
trainer = NeuralUtilityTrainer(users=X_train[:, 0].reshape(-1,1), items=X_train[:, 1:].reshape(-1,1),
                               y_train=y_train, model=mlp, loss=loss_logit,
